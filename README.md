...
https://github.com/Curiosity-Development/curiositycoin-wallet-backend-js

# CuriosityCoin-wallet-backend

Provides an interface to the CuriosityCoin network, allowing wallet applications to be built.
...
Note that you will need to prefix them all with `WB.` to access them, if you are not using typescript style imports, assuming you imported with `const WB = require('curiositycoin-wallet-backend-js')`.


### Javascript

```javascript
const WB = require('curiositycoin-wallet-backend-js');
...
```typescript
import { WalletBackend, Daemon, IDaemon } from 'curiositycoin-wallet-backend-js';
